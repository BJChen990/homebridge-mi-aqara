...
function isError(response: SimpleResponse<any>): response is SimpleResponseError {
...
export class MiIODevice {
  ...
  private waitQueue: { [addressPortHash: string]: WaitingRequest[] } = {};
    ...
    this.client.addMessageHandler((message, { address, port }) => {
      ...
      const responseId =
        packet.data.byteLength > 0
          ? (JSON.parse(packet.data.toString()) as SimpleResponseSuccess<any>).id
          : undefined;

      const hash = MiIODevice.getWaitQueueHash(address, port);
      if (!this.waitQueue[hash])  {
        return;
      }
  ...
  private async sendImpl(packet: Packet, requestId?: number) {
    const promise = new Promise<Packet>((resolve, reject) => {
      const hash = MiIODevice.getWaitQueueHash(this.address, this.port);
    ...
    await this.client.send(serialize(packet, this.token), this.address, this.port);
    return promise;
  }

  async send<A, R>(method: string, params: A): Promise<SimpleResponseSuccess<R>> {
    await this.maybeHandshake();
    const requestId = ++this.counter;
    const packet: Packet = {
      deviceId: Preconditions.checkExists(this.deviceId),
      timestamp:
        Math.floor((Date.now() - Preconditions.checkExists(this.lastHandshake)) * 0.001) +
